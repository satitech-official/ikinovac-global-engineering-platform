...
const catalogueImageVersion = '20260924d';
...
const baseProducts = catalogueCategories.flatMap(category => category.items.map((item, index) => {
  ...
  const rawImage = getImagePath(id, category.image);
  const suppliedImage = rawImage ? `${rawImage}${rawImage.includes('?') ? '&' : '?'}v=${catalogueImageVersion}` : rawImage;
...
export const productHref = product => `/products/${product.categorySlug}/#product-${product.slug}`;
