...
export async function createRFQPdf(rfq) {
  ...
  let companyLogo = null; const companyLogoBytes = await imageBytes(companyLogoPath); if (companyLogoBytes) { try { companyLogo = await pdf.embedJpg(companyLogoBytes); } catch { companyLogo = null; } }
  ...
  if (companyLogo) {
    ...
    const logoWidth = companyLogo.width * logoRatio; const logoHeight = companyLogo.height * logoRatio;
    page.drawRectangle({ x: logoBox.x - 3, y: logoBox.y - 3, width: logoBox.width + 6, height: logoBox.height + 6, color: palette.white });
    page.drawImage(companyLogo, { x: logoBox.x + (logoBox.width - logoWidth) / 2, y: logoBox.y + (logoBox.height - logoHeight) / 2, width: logoWidth, height: logoHeight });
  }
