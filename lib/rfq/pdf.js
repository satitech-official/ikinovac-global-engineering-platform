...
export async function createRFQPdf(rfq) {
  ...
  ensure(120, 'Project requirement');
  line();
  y -= 20;
  label('Overall project requirement', margin, y);
  y -= 18;

  const requiredLines = wrap(rfq.requirement, regular, 8.5, pageSize[0] - 2 * margin - 24);
  const requiredHeight = Math.max(76, requiredLines.length * 12.5 + 24);
  if (y - requiredHeight < 76) {
    addPage('Project requirement / continued');
  }
