    ...
    <main>
      ...
      <header className="site-header">
        ...
        <div className="main-nav">
          <a className="brand" href="#home" aria-label="IKINOVAC Global home" onClick={() => goTo("home")}>
            <span className="brand-mark"><img src={publicAsset("brand/ikinovac-logo-sharp.png")} alt="IKINOVAC Global logo" /></span>
            <span className="brand-text"><strong>IKINOVAC</strong><b>GLOBAL</b><small><span>ENGINEERING SOLUTIONS.</span><span>GLOBAL IMPACT.</span></small></span>
          </a>
      ...
      <section id="products" className="products-section section-shell">
        ...
        <div className="home-catalogue-grid">{homeCatalogueProducts.map((product, index) => <article className="home-catalogue-card" key={product.id}>
          <a className="home-catalogue-image" href={`/products/${product.categorySlug}/${product.slug}`} aria-label={`View ${product.name} details`}><img src={product.image} alt={`${product.name} industrial product`} loading="lazy" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = product.fallbackImage; }} /><span>{String(index + 1).padStart(2, "0")}</span><small>{product.category}</small></a>
      ...
      <section id="company" className="company-section section-shell dark-section">
        <div className="company-media"><div className="company-photo"><img className="company-logo-image" src={publicAsset("brand/ikinovac-logo-sharp.png")} alt="IKINOVAC Global — Engineering Solutions. Global Impact." /></div><div className="media-annotation"><span>EST.</span><strong>ENGINEERING-LED</strong><p>GLOBAL / RESPONSIVE / RELIABLE</p></div><div className="corner-frame" /></div>
