...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="home-hero" onMouseMove={event => { if (!window.matchMedia('(pointer: fine)').matches) return; const bounds = event.currentTarget.getBoundingClientRect(); setHeroOffset({ x: ((event.clientX - bounds.left) / bounds.width - .5) * -10, y: ((event.clientY - bounds.top) / bounds.height - .5) * -8 }); }} onMouseLeave={() => setHeroOffset({ x: 0, y: 0 })}>
      ...
      <div className="hero-section-number" aria-hidden="true">00</div>
      <div className="hero-technical-label" aria-hidden="true"><span>GLOBAL INDUSTRIAL SUPPLY</span><b>00 / 2026</b></div>
    ...
    <section className="global-network-section" data-reveal>
      <div className="network-copy reveal-up"><p className="eyebrow light">03 / GLOBAL SUPPLY NETWORK</p><h2>ONE PARTNER.<br /><em>GLOBAL SOLUTIONS.</em><br />ENDLESS POSSIBILITIES.</h2><p>From first enquiry through documented delivery, technical context stays connected to the supply conversation.</p><Link href="/global-presence" className="text-arrow light">Explore global capability <span>→</span></Link></div>
    ...
    <section className="company-intro-section" data-reveal>
      ...
      <div className="company-intro-copy reveal-up"><p className="eyebrow">04 / IKINOVAC STORY</p><h2>Engineering expertise.<br /><em>Global capability.</em></h2><p>We connect product understanding, sourcing intelligence and project coordination in one considered workflow.</p><div className="capability-lines">{['Founded by engineers', 'Global reach', 'Quality assured', 'Customer focused'].map((item, index) => <span key={item}><b>0{index + 1}</b>{item}<i>↗</i></span>)}</div><Link href="/company" className="text-arrow">Meet IKINOVAC <span>→</span></Link></div>
    </section>

    <section className="directory-section" data-reveal><div className="directory-heading reveal-up"><div><p className="eyebrow light">05 / INDUSTRIAL PRODUCT DIRECTORY</p><h2>A WORLD OF<br /><em>ENGINEERED</em><br />POSSIBILITIES.</h2></div><p>Explore product families as a connected industrial system—built for RFQ-led discovery, not a generic storefront.</p></div><div className="directory-grid reveal-clip">{featured.map((category, index) => <Link key={category.slug} href={`/products/${category.slug}`} className={`directory-card card-${index + 1}`} style={{ backgroundImage: `linear-gradient(180deg,rgba(13,23,20,.05),rgba(13,23,20,.9)),url(${assetUrl(category.image)})` }}><p>{category.number} / {category.items.length} FAMILIES</p><h3>{category.name}</h3><span>{category.summary}</span><b>Explore range <i>→</i></b></Link>)}</div><Link className="button button-gold directory-button reveal-up" href="/products">View all {catalogueCategories.length} categories <span>→</span></Link></section>

    <HomeProductShelf products={shelfProducts} />

    <CompleteSystem />

    <section className="process-section"><div><p className="eyebrow light">08 / ENGINEERING & PROCUREMENT PROCESS</p><h2>A CLEARER ROUTE<br />FROM <em>REQUIREMENT</em><br />TO DELIVERY.</h2><p>Technical and commercial conversations become one accountable supply path.</p></div><ol>{processSteps.map((step, index) => <li key={step} className={index === activeProcess ? 'active' : ''} onMouseEnter={() => setActiveProcess(index)}><b>{String(index + 1).padStart(2, '0')}</b><span>{step}</span><i>→</i></li>)}</ol></section>

    <section className="industries-section"><div className="section-title"><p className="eyebrow">09 / INDUSTRIES</p><h2>BUILT TO OPERATE<br /><em>WHERE IT COUNTS.</em></h2><Link href="/industries" className="text-arrow">View all industries <span>→</span></Link></div><div className="industry-rows">{industries.slice(0, 6).map(industry => <Link key={industry.name} href="/industries"><b>{industry.number}</b><h3>{industry.name}</h3><p>{industry.description}</p><i>↗</i></Link>)}</div></section>

    <section className="quality-section"><div className="quality-line" /><p className="eyebrow">10 / RESPONSIBLE SUPPLY</p><h2>DETAILS THAT<br /><em>TRAVEL WITH IT.</em></h2><div><p>Technical requirements, documentation and inspection context can be considered from the first project conversation.</p><Link className="button button-dark" href="/contact">Speak to the project desk <span>→</span></Link></div></section>

    <KnowledgePreview />

    <section className="home-contact-cta" style={{ backgroundImage: `linear-gradient(90deg,rgba(13,23,20,.93),rgba(13,23,20,.58)),url(${assetUrl('/assets/industry/procurement.jpg')})` }}><p className="eyebrow light">13 / START A PROJECT</p><h2>WHAT ARE YOU<br /><em>WORKING ON?</em></h2><p>Send the brief you have. A clear technical conversation is the next step.</p><Link className="button button-gold" href="/contact">Speak to the project desk <span>→</span></Link></section>
  </>;
}
