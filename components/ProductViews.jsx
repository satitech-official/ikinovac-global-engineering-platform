...
const productDetailVisualStyle = product => ({
  backgroundColor: '#071912',
  backgroundImage: `url(${assetUrl(product.cardImage || product.images[0])})`,
  backgroundPosition: product.cardImagePosition || 'center',
...
export function CategoryView({ category, products }) {
  ...
  return <>
    <section className="category-hero" aria-label={category.imageAlt || `${category.name} industrial equipment`} style={{ backgroundImage: `linear-gradient(90deg,rgba(13,23,20,.93),rgba(13,23,20,.45)),url(${assetUrl(category.image)})` }}><p className="eyebrow light">{category.number} / PRODUCT CATEGORY</p><h1>{category.name}</h1><p>{category.summary} Browse product families, then send the requirement directly to the project desk.</p><a href="#families" className="text-arrow light">Explore the range <span>↓</span></a></section>
    <section className="category-families" id="families"><div className="section-title"><div><p className="eyebrow">PRODUCT FAMILIES / {String(products.length).padStart(2, '0')}</p><h2>Specified around<br /><em>the requirement.</em></h2></div><p>Review the available product families and send the relevant requirement directly to the IKINOVAC project desk.</p></div><div className="family-card-grid">{products.map(product => <article className="family-product-card" id={`product-${product.slug}`} key={product.id}><div className="family-product-shell"><div className="family-product-image" role="img" aria-label={product.imageAlt} style={productVisualStyle(product)}><p>{category.number} / {String(product.order).padStart(2, '0')}</p><span>{product.family}</span></div><div className="family-product-copy"><p>{category.name}</p><h3>{product.name}</h3><span>{product.description}</span><button type="button" className="family-rfq-button" onClick={() => openQuote(product)}>Request quote <i>→</i></button></div></div></article>)}</div></section>
    <section className="industry-note"><p className="eyebrow">GLOBAL PROCUREMENT / RFQ READY</p><h2>Source {category.name.toLowerCase()} with <em>clear project context.</em></h2><p>For a stronger sourcing review, include the required product family, quantity, applicable standards, material or configuration, documentation needs, final destination and target delivery timing. IKINOVAC GLOBAL reviews the requirement against the published category scope before the commercial conversation moves forward.</p><Link href="/contact/" className="button button-dark">Send a {category.name} RFQ <span>→</span></Link></section>
...
export function ProductDetailView({ product, category }) {
  ...
  return <>
    <section className="product-detail-hero"><div className="product-gallery"><div className="product-detail-artwork" role="img" aria-label={product.imageAlt} style={productDetailVisualStyle(product)}><span>{product.categoryNumber} / {String(product.order).padStart(2, '0')}</span></div><p>IMAGE / REPRESENTATIVE PRODUCT FAMILY</p></div><div className="product-summary"><nav className="breadcrumb"><Link href="/products">Products</Link><span>/</span><Link href={`/products/${category.slug}`}>{category.name}</Link><span>/</span><b>{product.name}</b></nav><p className="eyebrow">{product.family} / PRODUCT FAMILY</p><h1>{product.name}</h1>{product.code && <p className="product-code">{product.code}</p>}<p>{product.description}</p><div className="product-applications"><b>APPLICATIONS</b>{product.applications.length ? product.applications.map(item => <span key={item}>{item}</span>) : <span>Application guidance available on request</span>}</div><button className="button button-dark" onClick={() => openQuote(product)}>REQUEST A QUOTE <span>→</span></button><small>Share the requirement you know. IKINOVAC confirms final specification, availability and commercial terms after review.</small></div></section>
    ...
    <section className="detail-info-section"><article><p className="eyebrow">DOCUMENTS</p><h2>Documentation<br /><em>when relevant.</em></h2>{product.documents.length ? product.documents.map(document => <a href={document.url} key={document.url}>{document.title} →</a>) : <p>No document is published for this product family. Request available documentation from the project desk.</p>}</article><article><p className="eyebrow">PROJECT DESK</p><h2>One product.<br /><em>Clear requirement.</em></h2><p>Tell us the essential product and project context. IKINOVAC will respond with the appropriate technical and commercial review.</p><button className="button button-dark" onClick={() => openQuote(product)}>Request a quote <span>→</span></button></article></section>
